...
const gsnHandler = {
    ...
    uploaddata:
        async function (req, res) {
            try {
                ...
                // Log parsed table data before saving
                console.log("Backend: parsedTableData before save:", parsedTableData);
                ...
                console.log("Backend Saving (GSN):", newInventory);
    ...
    updateByParty: async function (req, res) {
        try {
            ...
            if (!partyName) return res.status(400).json({ message: 'Party name is required' });
            const result = await gsnEntries.updateMany({ partyName }, { $set: updateData });
